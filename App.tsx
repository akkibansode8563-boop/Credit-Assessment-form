...
const App: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // 2. Submit to Server (GAS Bridge)
      await new Promise((resolve, reject) => {
        const gas = (window as any).google?.script?.run;
        if (gas) {
          gas.withSuccessHandler((res: any) => {
            if (res && res.success) resolve(res);
            else reject(new Error(res?.error || "Server failed to process the request."));
          })
          .withFailureHandler((err: any) => reject(new Error(err.message || "Failed to communicate with Google Servers.")))
          .processSubmission({ ...formData, pdfData: pdfBase64 });
        } else {
          // If not in GAS environment, fail with descriptive message
          reject(new Error("Google Script environment not detected. This application must be deployed as a Web App in Google Apps Script to send emails."));
        }
      });

      setIsSuccess(true);
    } catch (err: any) {
      setErrorMessage(err.message || "Transmission failed. Verify your internet connection and App Script deployment.");
    } finally {
  ...
  return (
    <div className="min-h-screen pb-20">
      ...
      <main className="max-w-6xl mx-auto px-4 pt-48">
        ...
        {isSuccess ? (
          <div className="bg-white p-12 rounded-3xl shadow-2xl border border-slate-200 text-center space-y-8 max-w-lg mx-auto animate-in zoom-in-95 duration-500">
            ...
            <div className="space-y-3">
              ...
              <p className="text-slate-500 leading-relaxed">
                The assessment for <strong>{formData.companyName}</strong> has been successfully processed. 
                An email with the PDF attachment has been dispatched to the credit assessment desk.
              </p>
            </div>
            <button 
              onClick={handleResetForm}
              className="w-full bg-slate-900 text-white py-4 rounded-xl font-bold hover:bg-slate-800 transition-all shadow-xl flex items-center justify-center gap-2"
            >
              <Plus className="w-5 h-5" /> New Assessment Form
            </button>
          </div>
        ) : (
          <form onSubmit={handleSubmit} className="space-y-8">
            ...
            <div className="sticky bottom-8 z-40 flex justify-center pt-8">
              <div className="bg-white/90 backdrop-blur-xl p-4 rounded-3xl shadow-2xl border border-white/50 flex flex-col md:flex-row gap-4 w-full max-w-lg">
                <button 
                  type="button" 
                  onClick={handleExportOnly}
                  className="flex-1 flex items-center justify-center gap-2 bg-slate-100 text-slate-700 py-4 rounded-2xl font-black text-xs uppercase hover:bg-slate-200 transition-all border border-slate-200"
                >
                ...
                <button 
                  ...
                  className="flex-1 flex items-center justify-center gap-2 bg-slate-900 text-white py-4 rounded-2xl font-black text-xs uppercase hover:bg-slate-800 shadow-xl disabled:opacity-50 transition-all group"
                >
                  {isSubmitting ? (
